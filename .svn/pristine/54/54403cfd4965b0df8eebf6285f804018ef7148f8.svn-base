<template>
    <div class="pick-details pb-lg">
        <mt-header fixed :title="`PL${this.pickingNumber} 移库上架单`">
            <span slot="left" @click="backpath">
                <i class="mintui mintui-back go-back"></i><span>返回</span>
            </span>
            <div slot="right" class="partition" @click="changeState" v-if="active==='tab-container1'||active==='tab-container3'">上架路径</div>
        </mt-header>
                        ...
                        <div v-for="(row,index) in detailsSortData" :key="row.rindex" @click="get_line(row,index)">
                            <table>
                                <tr class="all-table"
                                    ...
                                    <td style="width: 30%">{{row.new_cargo_code}}</td>
                ...
                <form class="form">
                    ...
                    <div class="outbox">
                        ...
                        <div class="request_btn request_btn_t" v-if="imgshow">
                            <request-button v-if="active==='tab-container3'" :request="make_off">确认上架
                            </request-button>
                        </div>
                    </div>
                </form>
...
<script>
    import {api_warehouse_cargo_shift_detail,api_warehouse_cargo_shift_shelves} from '../../api/putaway-shifting';
        ...
        methods:{
            backpath(){
                if(this.active==='tab-container3'){
                    ...
                    this.$router.push({path:'/putaway-shifting',query:{warehouse_id:this.$route.query.warehouse_id}})
                }
            },
            changeState(){
                this.active='tab-container2'
            ...
            get_details(orderId){
                ...
                this.$http(api_warehouse_cargo_shift_detail,data).then(res=>{
                    ...
                    this.line_now = this.detailsData.findIndex(row =>{
                        this.new_cargo_id = row.new_cargo_id
            ...
            get_line(item,num){
                ...
                this.detailsData.forEach((row,index)=>{
                    if(num===index){
                        this.lineData = {
                            warehouse_cargo_code:row.new_cargo_code,
                            sku:row.sku,
                            id:row.id,
                        ...
                        this.sku = '';
                        this.warehouse_cargo = row.new_cargo_code;
                ...
                if(val){
                    ...
                    if(!!find){
                        if(Number(this.unshelf) > Number(find.quantity)){
                            this.unshelf_error = `可上架数量最大为${find.quantity}`;
                            this.quantitySelect = true;
                        }else{
                            this.unshelf_error = '';
                        }
            ...
            make_off(){
                ...
                return this.$http(api_warehouse_cargo_shift_shelves,data).then(res=>{
                    this.$refs.sku.$refs.input.select();
                    ToastMessage({message:`${this.sku}已上架`,position: 'top',duration: 1500});
                    console.log('this.detailsData.length',this.detailsData.length);
                    if(res.message.includes(`已完成上架`) || res.message.includes(`操作完成`)){
                        MessageBox({
                            title: '提示',
                            message: '您已全部上架，已完成拣货',
                            confirmButtonText: `好的，我知道了`
                        });
                        this.$router.push({
                            path:"/putaway-shifting",
                            query:{
                                warehouse_id:this.$route.query.warehouse_id
                            }
                        });
