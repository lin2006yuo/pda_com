<template>
        <div class="putaway-order pb-lg">
            <mt-header fixed title="集包交接">
                <router-link to="/menu" slot="left">
                    <i class="mintui mintui-back go-back"></i><span>返回</span>
                </router-link>
            </mt-header>
            <div class="content">
                <button-input :mt="{'margin-top':'33px'}"lineName="集包单号" :inputSelect="inputSelect" :error-message="errorMessage" v-model="code" @submit="submit"></button-input>
                <tab-table titleName="已交接单号" :code-count="codeCount">
                    <tr><td>交接单号</td><td>邮寄方式</td></tr>
    ...
    <script>
        ...
        import{api_package_collection} from '../../api/putaway-order';
        /*
         * 后端:詹熏欣
         * */
        export default{
            ...
            methods:{
            	...
            	submit(){
                    this.$nextTick(()=>{
                        ...
                        this.$http(api_package_collection,this.code).then(res=>{
                            Toast({message:res.message,position: 'top',duration: 1500});
                            let data = {
                                code: this.code,
